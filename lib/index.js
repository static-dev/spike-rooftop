...
module.exports = class Rooftop {
  ...
  apply (compiler) {
    compiler.plugin('run', (c, d) => this.run(compiler, c, d).done())
    compiler.plugin('watch-run', (c, d) => this.run(compiler, c, d).done())
    compiler.plugin('emit', (compilation, done) => {
      if (this.json) {
        const src = JSON.stringify(compiler.options.spike.locals.rooftop, null, 2)
  ...
  run (compiler, compilation, done) {
    return W.reduce(this.contentTypes, (m, ct) => {
      ...
      compiler.options.spike.locals.rooftop = res
      done()
    })
  }
